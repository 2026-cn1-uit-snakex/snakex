...
let
  ...
  # nixprint generates the agent client files and rewrites them on shell entry.
  # Their markdown shape is not ours to fix.
  generatedAgentPaths = [
    "^\\.agents/"
    "^\\.claude/"
    "^\\.opencode/"
...
in
{
  workspace = {
    ...
    # Implementation expert for the C++ applications in this workspace. The
    # write boundary keeps it inside the snake application repository.
    blueprint.polyrepo.implementationExperts.apps-cpp = {
      description = "Implement C++ application changes in apps/snake with the workspace C++ toolchain and standard";
      writePaths = [ "apps/snake" ];
  ...
  git-hooks.hooks = {
    ...
    # Do not lint or rewrite generated files. Both hooks reject the agent files
    # nixprint emits, which would block every commit that touches them.
    markdownlint.excludes = generatedAgentPaths;
    end-of-file-fixer.excludes = generatedAgentPaths;
  };
